...
public class VelocityEngineFactory {
     ...
     * resource location via the "resourceLoaderPath" property. Setting it here is
     * just necessary when using a non-file-based resource loader.
     * 
     ...
     * "file". If the specified resource cannot be resolved to a
     * {@code java.io.File}, a generic SpringResourceLoader will be used under the
     * name "spring", without modification detection.
     * <p>
     ...
     * "file.resource.loader.modificationCheckInterval" property. By default, the
     * file timestamp is checked on every access (which is surprisingly fast). Of
     * course, this just applies when loading resources from the file system.
     * <p>
     * To enforce the use of SpringResourceLoader, i.e. to not resolve a path as
     * file system resource in any case, turn off the "preferFileSystemAccess" flag.
     ...
     * default is DefaultResourceLoader. Will get overridden by the
    ...
    /**
     * Set whether to prefer file system access for template loading. File system
     ...
     * "resourceLoaderPath" as file system resource (which will work for expanded
     * class path resources and ServletContext resources too).
     * <p>
     * Default is "true". Turn this off to always load via SpringResourceLoader
     * (i.e. as stream, without hot detection of template changes), which might be
     * necessary if some of your templates reside in an expanded classes directory
    ...
    /**
     * To be implemented by subclasses that want to perform custom post-processing
     * of the VelocityEngine after this FactoryBean performed its default
     * configuration (but before VelocityEngine.init).
     * <p>
     * Called by {@code createVelocityEngine()}.
     * 
